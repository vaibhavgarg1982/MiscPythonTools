...
def animate(i, *fargs): 
    ...
    for index, phase in enumerate(phases, start=1,):
        xs.append([xs[index-1][i] + (hmag[index-1])*math.cos((index+1)*ang+phase) for ang in angles])
        ys.append([ys[index-1][i] + (hmag[index-1])*math.sin((index+1)*ang + phase) for ang in angles])
...
anim = FuncAnimation(fig, animate, fargs=fargs, init_func = init, 
                     frames = len(angles), interval = 10, blit = True) 
